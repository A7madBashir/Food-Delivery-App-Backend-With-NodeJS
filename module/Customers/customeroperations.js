...
Router.route("/register").post((req, res, done) => {
  let Signup = req.body;
...
Router.route("/Login").post((req, res, done) => {
  let Login = req.body;
  Checkcus(Login)
    .then((result) => {
      if (!result) {
...
Router.route("/ShowCustomer/:id").get(
  passport.authenticate("jwt", { session: false }),
  async (req, res) => {
